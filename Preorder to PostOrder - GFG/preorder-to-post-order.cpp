...
class Solution{
public:
 Node* solve(int mini,int maxi,int pre[],int size,int& ind)
{
    if(ind>=size)
    {
        return NULL;
    ...
    root->left = solve(mini,root->data,pre,size,ind);
    root->right = solve(root->data,maxi,pre,size,ind);
    return root;
}
    ...
    Node* post_order(int pre[], int size)
    {
       ...
       int maxi = INT_MAX;
       int ind = 0;
       return solve(mini,maxi,pre,size,ind);
    }
};
