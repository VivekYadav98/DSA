...
public:
    ...
    int findSum(Node* root,int& cnt)
    {
        ...
        cnt++;
        return root->data + findSum(root->left,cnt) + findSum(root->right,cnt);
    }
    ...
    int solve(int target,Node* root)
    {
        ...
        int cnt = 0;
        if(isBST(root,INT_MIN,INT_MAX) && findSum(root,cnt) == target)
        {
            ans = min(ans,cnt);
        }
        
        return min(solve(target,root->left),solve(target,root->right));
    }
    
    int minSubtreeSumBST(int target, Node *root) {
        int res = solve(target,root);
         if(res == INT_MAX)
         {
             return -1;
         }
